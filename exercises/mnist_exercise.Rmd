...
This exercise follows the MNIST example given in https://keras.rstudio.com/ with slight modifications. You will learn how to build a simple neural network which learns to classify handwritten digits.

**Prerequisities**

Please install the r package keras in advance:  
...
Plot an image in grey scale using the `image()` function.
...
```{r}
indices <- sample(1:nrow(X), 16)
par(mfrow = c(4, 4), mar = c(0,0,0,0), yaxt = "n", xaxt = "n")
for(i in indices)
  "CHANGE ME!" # plot i:th image
```
...
*hint* 
Complete the network definition by defining the input shape of the first dense layer. This should correspond to the shape of the input data, ignoring the sample size dimension. e.g. if our data has 3 features, this should be a length one integer vecor with the value 3.
...
The predictions are given as a vector of probabilities for each label ("soft" predictions). A simple way to evaluate model performance is by converting to "hard" predictions and then comparing the predictions to the actual labels.  
