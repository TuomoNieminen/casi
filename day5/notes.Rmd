...
*ref: Computer age statistical inference, pages 351 - 356.*
...
Without the nonlinearities, the neural network reduces to a generalized linear model.
...
We will define a neural network which produces probabilities for each of the labels.

### Output layer

In this example the output layer of the network has 10 nodes, one for each of the possible class labels. The final nonlinear transformation $g^{(K)}$ is the *softmax function*  
...
*ref: Computer age statistical inference, page 355*
...
where $\lambda$ is a tunign parameter for the penalty function $J(W)$. For example, using the square loss and no penalty ($\lambda = 0$), the cost becomes
...
A cost function can be derived via maximum likelihood estimation (possibly adding a penalty term). An MLE is a type of objective function, a general expression for the function to optimize.
...
To fit the model, the objective is to minimize the cost function with respect to $W$. There is no analytical solution, so we resort to numerical methods. Given the gradient $\Delta C(W)$ of the cost function, the gradient descent update on the weights is given by
...
*ref: Computer age statistical inference, page 356.*
