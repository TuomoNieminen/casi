...
## Example: A neural network for classification
...
We will define a neural network which produces probabilities for each of the labels. In this example the output layer of the network has 10 nodes, one for each of the possible class labels. The final nonlinear transformation $g^{(K)}$ in the **output layer** is the *softmax function*  
...
$$
L(f(x_i | W), y_i) = \sum_{j = 0, .., 9}y_{ij} \cdot log (\hat{p_{ij}}).
$$
...
$$
also known as Ordinary Least Squares (OLS). A cost function can be derived via maximum likelihood estimation (possibly adding a penalty term). An MLE is an example of an *objective function*, a general expression for the function to optimize (maximize or minimize).
